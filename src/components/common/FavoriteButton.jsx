...
export default function FavoriteButton({ listingId, className = '' }) {
  ...
  return (
    <button
      ...
      style={
        favorited
          ? { borderColor: 'rgba(239,68,68,0.5)', boxShadow: '0 4px 16px rgba(239,68,68,0.25), inset 0 1px 0 rgba(255,255,255,0.6)' }
          : undefined
      }
    ...
    >
      {loading ? (
        ...
        <Heart size={16} className={favorited ? 'text-red-500' : 'text-[#64748B]'} fill={favorited ? 'currentColor' : 'none'} />
      )}
    </button>
  );
}